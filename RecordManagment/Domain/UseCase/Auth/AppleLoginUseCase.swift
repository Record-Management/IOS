...
struct DefaultAppleLoginUseCase: AppleLoginUseCase {
    ...
    func login(authUserData: AuthUserData) async -> UserState {
        guard let result = await repository.login(authUserData: authUserData) else {
            return .login
        }
        
        switch result {
        case .success(let response):
            debugPrint("Apple login result : \(response)")
            guard let statusCode = response.statusCode else {
                debugPrint("statusCode가 존재하지 않습니다.")
                return .login
            }
        ...
        case .failure(let err):
            debugPrint("AppleLogin Error : \(err)")
            return .login
        }
    }
}
